...
RELEASE_TAR="$GEN_DIR/kitowall-${TAG}.tar.gz"
TMP_UI_DIR=""
...
curl --fail --location --retry 6 --retry-all-errors --retry-delay 2 "$ARCHIVE_URL" -o "$RELEASE_TAR"
SHA="$(sha256sum "$RELEASE_TAR" | awk '{print $1}')"
